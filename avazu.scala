...
var transformedData = pipelineModel.transform(data)
transformedData = transformedData.withColumn("label2", $"label".cast("int")).drop("label").withColumnRenamed("label2", "label")
val lr = new LogisticRegression()
lr.setFeaturesCol("featureVector")
lr.setLabelCol("label")
lr.setMaxIter(10)
val lrModel = lr.fit(transformedData)
lrModel.transform(transformedData.select("featureVector")).show(4)
